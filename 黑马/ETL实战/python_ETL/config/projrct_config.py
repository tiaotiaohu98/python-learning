...
# print(log_root_path)
log_name = f'\\executelogs\\pyetl-{time.strftime("%Y-%m-%d %H", time.localtime())}.log'
...
metadata_port = 33061
metadata_user = 'root'
metadata_password = '402191'  # 需要更改为自己的密码
...
# ################## --后台日志数据采集配置项-- ###################
# 待采集 日志 文件所在的目录
backend_logs_data_root_path = log_root_path
...
# 后台日志数据写出 csv 的根路径
logs_output_csv_root_path = log_root_path+"\\output\\backend_logs\\"
# 每一次运行，后台日志文件写出路径
...
# ==================目标CSV路径配置=============================================
# 商品数据写出 csv 根路径
barcode_output_csv_root_path = log_root_path+"\\output\\barcode/"
# 商品数据写出 csv 文件名
...
# =========================JSON=======================================================================
# json源文件路径配置
json_origin_path = root_path + "/source/ord_json/"

# ################## --订单JSON数据采集配置项-- ###################
# 订单表名称
target_orders_table_name = "orders"
# 订单表MySQL数据库建表语句列信息
target_orders_table_cols_name = \
...
# 订单详情建表列信息
target_orders_detail_table_cols_name = \
...
# 单个商品信息表
target_singleproduct_sold_table_name = "singleproduct_sold"
# 单个商品信息表列信息
target_singleproduct_sold_table_cols_name = \
    f"barcode VARCHAR(255) COMMENT '商品条码', " \
    f"category_id INT COMMENT '商品类别ID', " \
    f"name VARCHAR(255) COMMENT '商品名称', " \
    f"price_per DECIMAL(10, 5) COMMENT '实际售卖单价', " \
    ...
    f"unit_id INT COMMENT '商品单位ID(包、袋、箱、等)', " \
    f"count INT COMMENT '本单此商品卖出数量', " \
    f"PRIMARY KEY (barcode)"
    
    
# 订单数据写出 csv 的根路径
retail_output_csv_root_path = log_root_path+"output/orders/"
# 每一次运行，订单文件写出路径
...
retail_singleproduct_sold_output_csv_file_name = \
    f"singleproduct_sold-{time.strftime('%Y-%m-%d-%H_%M', time.localtime())}.csv"